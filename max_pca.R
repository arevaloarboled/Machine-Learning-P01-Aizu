...
train=as.data.frame(prcomp(t(train))$rotation)
coefficients=list()
for (j in unique(train_label)) {
	tmp=list()
	for (k in colnames(train)) {
		x=train[train_data[,i]==j,k]
...
test=as.data.frame(prcomp(t(test))$rotation)
#p_cl(test[15,],coefficients,as.character(test_data[1,i]))
#p_cl(test[15,],coefficients,as.character(test_data[11,i]))
#p_cl(test[15,],coefficients,as.character(test_data[21,i]))
P<-function(y,c,clss){
	q=0
	v=""
	for (k in clss) {
...
for (l in seq(1,length(test_label))) {
	predict=c(predict,P(test[l,],coefficients,as.character(unique(train_label))))
}
